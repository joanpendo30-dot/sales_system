...
if ($user['role'] === 'admin') {
    ...
    $businesses = [];
    if ($user['business_id']) {
        $stmt = $db->prepare("SELECT id, name FROM businesses WHERE id = ?");
        $stmt->execute([$user['business_id']]);
        $businesses = $stmt->fetchAll();
    }
}

// All stock rows, for the JS product picker (id, business_id, product_name, unit_price, quantity, quantity_sold)
$allStockJs = $db->query(
        ...
        $db->prepare(
            "INSERT INTO orders
                (customer_name, product_name, quantity, unit_price, total_amount,
                 order_date, status, business_id, created_by, created_by_name)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        )->execute([
            $customer_name, $product_name, $quantity, $unit_price, $total_amount,
            $order_date, $status, $business_id, $user['id'], $user['full_name']
        ]);

        // Deduct from matching stock, unless the order is cancelled
        if ($status !== 'cancelled') {
...
<?php if ($user['role'] !== 'admin' && empty($businesses)): ?>
    <div class="alert alert-warning">
        You are not assigned to a business yet. Contact your administrator before creating orders.
    </div>
<?php else: ?>
    ...
    <div class="card-body p-4">
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <?php else: ?>
                    <input type="text" class="form-control" value="<?= htmlspecialchars($businesses[0]['name']) ?>" disabled>
                    <input type="hidden" name="business_id" value="<?= (int)$businesses[0]['id'] ?>">
                <?php endif; ?>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="product_name" id="product_name" class="form-control"
                       ...
                       placeholder="Or type a product name manually" required>
            </div>
            ...
            <div class="row g-3 mb-3">
                ...
                <div class="col-4">
                    ...
                    <input type="number" name="unit_price" class="form-control" min="0" step="0.01"
                           value="<?= htmlspecialchars($_POST['unit_price'] ?? '0.00') ?>" required>
                </div>
                <div class="col-4">
                    <label class="form-label fw-semibold">Order Date <span class="text-danger">*</span></label>
                    <input type="date" name="order_date" class="form-control"
                           value="<?= htmlspecialchars($_POST['order_date'] ?? date('Y-m-d')) ?>" required>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="status" class="form-select">
                    <?php $statusVal = $_POST['status'] ?? 'pending'; ?>
